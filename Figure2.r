...
workdir <- "."
datafile <- file.path(workdir,"Figure2.csv")
...
tabletext <- cbind(c("Size/group",data$casecount),
                   c("Exposure\n group\n",data$controlcount),
                   c("Comparison\n group\n",data$blah),
                   c("# of cases (overall risk)\n(Exposure group)",data$case_group),
                   c("# of cases (overall risk)\n(Comparison group)",data$control_group),
                   
                   c("        HR (95% CI)",data$CI)
)

pdf(file.path(workdir,"Figure2.pdf"),  onefile=FALSE, width=14, height=6.5)
...
forestplot(labeltext=tabletext, 
          ...
          title ="Five-year risk of incident pancreatic cancer in patients with type 2 diabetes \n(comparison between propensity-score matched groups: GLP-1RAs vs other anti-diabetes medications)", 
